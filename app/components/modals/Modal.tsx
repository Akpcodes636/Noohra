...
// import useModalStore from "@/store/modal"; // adjust this path if needed
import NoticeModal from "./NoticeModal";
// import { useUserInfo } from "@/context/UserInfoContext"; // ✅ use the context
...
export default function TestModal() {
  ...
  const { setUserInfo } = useUserInfo(); // ✅ get setUserInfo from context
  ...
  const handleConfirm = () => {
    ...
    setUserInfo((prev) => ({
      ...prev,
      testTaker: selectedOption as "someone_else" | "myself" | null,
    }));

    if (selectedOption === "someone_else") {
      openSecondModal(); // show second modal
      return;
    }

    closeModal(); // close modal if "myself"
    router.push("/Gender"); // navigate to gender page
  };

  return (
    <>
      {isModalOpen && !secondModalOpen && (
        <div
        ...
        >
          <div className="flex items-center justify-center min-h-screen max-w-[343px] mx-auto">
            <div
              className={`w-full max-w-[343px] lg:max-w-[460px] rounded-[16px] ${
                showError ? "min-h-[382px]" : "min-h-[310px]"
              } bg-[#FFFFFF] p-[16px]`}
            >
              {/* Header */}
              ...
              <div className="mb-[24px]">
                {["myself", "someone_else"].map((option) => (
                  <div
                    key={option}
                    className={`${
                      selectedOption === option ? "border-2 border-[#BEEAE5]" : ""
                  ...
                  >
                    <div className="w-[10%] flex items-center justify-center">
                      <div className="w-[30px] h-[31px]">
                        <Image
                          src={option === "myself" ? "/image.svg" : "/humans.svg"}
                          alt={option}
                          width={500}
                          height={500}
                    ...
                    <div className="w-[90%]">
                      <h2 className="text-[#464442] capitalize">{option.replace("_", " ")}</h2>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="bg-[#FDF4DD] p-3 mb-4 rounded-md flex items-center gap-2 drop-shadow-[0px_2px_8px_0px_#DEA91A40]">
                  ...
                  <div>
                    <p className="text-[12px] text-[#7A5A0D]">
                      If you&apos;re taking this assessment on behalf of someone else, please use their
              ...
              <div className="flex items-end justify-end mt-2">
                <Button
                  ...
                  css={`w-[89px] h-[32px] rounded-[8px] ${
                    selectedOption ? "bg-[#15544D]" : "bg-gray-300 cursor-not-allowed"
                  } text-[#FFF]`}
                  fn={selectedOption ? handleConfirm : undefined}
                >
                  Okay
                </Button>
              </div>
